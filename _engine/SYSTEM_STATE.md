...
## ⚠️ 다음 대화 최우선 — 썸네일 카피 가독성 (사용자 강력 요청)
문제: 썸네일 카피가 길면(한 줄 15~18자) 폰트가 80px까지 작아져 모바일에서 안 읽힘.
원칙: "폰트를 줄이지 말고 줄을 늘려라". 구현 3가지:
1. 카피 짧게 강제 — 한 줄 6~10자. 프롬프트 3-1절/이미지7절에서 글자수 빡세게 제한.
   (도파민 자극 + 본문 요약 한 방. 예: "밥상 물가가 문제입니다"(X) → "밥상 물가가 문제"(O))
2. build_thumbnail_svg / build_stock_thumbnail_svg 의 fs(폰트) 로직 개선:
   - 현재: fs = 92 if max(len)<=11 else 80  → 길면 그냥 작아짐 (문제)
   - 개선: 폰트 하한(예 78px) 절대 보장 + 짧으면 초대형(110px+)
3. 2줄에 안 담기면 폰트 줄이지 말고 '3줄'로 자동 재배치(어절 단위). line1/line2 대신
   카피를 받아 빌더가 2~3줄로 나누고 줄수·최장줄길이로 폰트 결정하는 구조가 이상적.
   → 미리보기 여러 길이(짧/중/긴)로 반복 테스트 필요. 그래서 새 대화에서 집중.
관련 함수: common_utils.py build_thumbnail_svg(~360행), build_stock_thumbnail_svg(~400행)
